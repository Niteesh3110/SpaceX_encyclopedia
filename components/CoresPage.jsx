...
function CoresPage({ page, coresData }) {
  const [localCoresData, setLocalCoresData] = useState(coresData);
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const requestData = await getCoresData({ page });
      if (requestData?.boolean) {
        setLocalCoresData(requestData?.data);
      } else {
  ...
  useEffect(() => {
    if (localCoresData) console.log(localCoresData);
  }, [localCoresData]);
  ...
  async function handleSearch(query) {
    ...
    if (query.length > 1) {
      try {
        const requestData = await getCoresData({
          name: query,
          page: 1,
        });
        if (requestData?.boolean) {
          setLocalCoresData(requestData?.data);
          router.push("/cores/page/0");
        } else {
          console.log("Could not fetch data");
        }
      } catch (error) {
  ...
  const ResultCards = ({ id, name, status }) => (
    <>
  ...
  const Pages = ({ page, totalPages }) => {
    ...
    return (
      <>
        {page - 1 >= 0 ? (
          <Link href={`/cores/page/${page - 1}`} className="text-white">
        ...
        {page <= totalPages ? (
          <Link href={`/cores/page/${page}`} className="text-white">
        ...
        {page + 2 <= totalPages ? (
          <Link href={`/cores/page/${page + 2}`} className="text-white">
            <div className=" h-6 w-6 border rounded-sm text-center">
              {page + 2}
            </div>
          </Link>
          ...
          <div className="min-h-screen border w-full max-w-8xl flex-1 flex flex-col bg-none rounded-lg shadow-sm overflow-y-hidden z-20">
            <div className="h-screen w-full max-w-5xl mx-auto p-4 overflow-y-scroll">
              ...
              <div className="space-y-4 flex flex-wrap gap-5 justify-start items-center pl-24">
                {localCoresData?.docs && localCoresData?.docs.length !== 0 ? (
                  localCoresData.docs.map((data, index) => (
                    <ResultCards
                      key={data?.id}
                      id={data?.id}
                      name={data?.serial}
            ...
            <div className="mt-auto w-full max-w-4xl mx-auto">
              ...
              <div className="flex p-1 justify-center gap-1 z-20 bg-none">
                <Pages page={page} totalPages={localCoresData?.totalPages} />
              </div>
            </div>
          </div>
        </div>
