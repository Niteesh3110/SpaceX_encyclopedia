...
function RocketsPage({ page, rocketData }) {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const requestData = await getRocketData({ page });
      if (requestData?.boolean) {
        setLocalRocketData(requestData?.data);
      } else {
  ...
  useEffect(() => {
    if (localRocketData) console.log("localRocketData", localRocketData);
  }, [localRocketData]);
  ...
  async function handleSearch(query) {
    ...
    if (query.length > 1) {
      try {
        const requestData = await getRocketData({
          name: query,
          page: 1,
        });
        if (requestData?.boolean) {
          setLocalRocketData(requestData?.data);
          navigate.push("/rockets/page/1");
        } else {
          console.log("Could not fetch data");
        }
      } catch (error) {
      ...
      <div className="w-full h-full p-4 border rounded-lg bg-black/50 shadow-sm hover:shadow-md transition-shadow flex gap-12 text-white items-center">
        ...
        <div className="flex flex-col justify-start items-start">
          ...
          <div className="flex justify-start gap-5">
            <Link href={`/rockets/${id}`} className="hover:text-gray-500 py-2">
              Details →
            </Link>
          </div>
        </div>
  ...
  const Pages = ({ page, totalPages }) => {
    ...
    return (
      <>
        {page - 1 >= 0 ? (
          <Link href={`/rockets/page/${page - 1}`} className="text-white">
        ...
        {page <= totalPages ? (
          <Link href={`/rockets/page/${page}`} className="text-white">
        ...
        {page + 2 <= totalPages ? (
          <Link href={`/rockets/page/${page + 2}`} className="text-white">
            <div className=" h-6 w-6 border rounded-sm text-center">
              {page + 2}
            </div>
          </Link>
          ...
          <div className="min-h-screen border w-full max-w-8xl flex-1 flex flex-col bg-none rounded-lg shadow-sm overflow-y-hidden z-20">
            <div className="h-screen w-full max-w-5xl mx-auto p-4 overflow-y-scroll">
              ...
              <div className="space-y-4">
                {localRocketData?.docs && localRocketData?.docs.length !== 0 ? (
                  localRocketData.docs.map((data, index) => (
                    <ResultCards
                      key={data?.id}
                      id={data?.id}
                      name={data?.name}
            ...
            <div className="mt-auto w-full max-w-4xl mx-auto">
              ...
              <div className="flex p-1 justify-center gap-1 z-20 bg-none">
                <Pages page={page} totalPages={localRocketData?.totalPages} />
              </div>
            </div>
          </div>
        </div>
